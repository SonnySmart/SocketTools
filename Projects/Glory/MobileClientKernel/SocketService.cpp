...
bool CSocketService::SendSocketData(unsigned short wMain, unsigned short wSub, const void* pData, unsigned short wDataSize)
{
	...
	if (this->m_bServe && this->m_bRun && this->m_pSocket && wDataSize < sizeof(TcpBuffer.cbBuffer))
	{
...
void CSocketService::SetWaitTime(float time)
{
...
long long CSocketService::getCurrentTime()
{
	struct timeval tv;
#ifdef WIN32
...
void CSocketService::OnRun()
{
	...
	catch (SocketServiceRunErrorCode e)
	{
		if (this->m_bServe)
		{
			...
			bRet = pSocketEvent->HanderMessage(MSG_SOCKET_ERROR, this->m_nHandler, 0, 2, NULL, 0);
			//assert(!szError);
			printf("s",szError);
		}
	}
