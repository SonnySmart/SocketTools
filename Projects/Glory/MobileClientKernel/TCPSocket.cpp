...
bool CTCPSocket::OnEventSocketConnetByIP(const char *ip, WORD port)
{
	...
	int nRet1;
	bool result;
	const char *szIp1 = NULL;
	struct addrinfo *AddrInfo;
	sockaddr_in6 sin6;
	unsigned int cbIpByte[4];
	char szIpV6[240];
	sockaddr_in sin;
	char szPort[16];
	...
	if (getaddrinfo("www.mpj99.com", 0, &this->addrCriteria, &AddrInfo))
	{
		printf("OnEventSocketConnetByIP getaddrinfo fail %s", "www.mpj99.com");
		return false;
	}
